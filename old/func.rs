...
use crate::runtime::{TrapReason, VMContext, VMFunctionImport, VMVal};
use crate::{runtime, Store, Val, MAX_WASM_STACK};
use core::mem;
...
impl Func {
    ...
    unsafe fn call_unchecked_raw(
        ...
        // variable.
        let stack_pointer = runtime::arch::get_stack_pointer();
        let wasm_stack_limit = stack_pointer - MAX_WASM_STACK;
        let prev_stack = unsafe {
            mem::replace(
                &mut *vmctx
                    .byte_add(module.vmoffsets().static_.stack_limit as usize)
                    .cast::<usize>(),
                wasm_stack_limit,
            )
        };

        unsafe { crate::placeholder::signals::register_signal_handler() };
        
        let res = runtime::catch_traps(
            vmctx,
            module.vmoffsets().static_.clone(),
            |caller| {
