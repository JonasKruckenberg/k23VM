...
use crate::traps::{trap_for_offset, Trap};
use core::ops::Range;
use core::{fmt, slice};
use object::{File, Object, ObjectSection};
use crate::compile::{FunctionLoc, ELF_K23_ADDRESS_MAP, ELF_K23_INFO, ELF_K23_TRAPS, ELF_TEXT, ELF_WASM_DATA, ELF_WASM_DWARF, ELF_WASM_NAMES};
...
impl CodeMemory {
    pub fn new(vec: MmapVec<u8>) -> Self {
        ...
        let mut address_map = Range::default();
        ...
        for section in obj.sections() {
            ...
            match name {
                ...
                ELF_K23_ADDRESS_MAP => address_map = range,
    ...
    pub fn publish(&mut self) -> crate::Result<()> {
        ...
        unsafe {
            self.inner.make_readonly(0..self.inner.len())?;
    ...
    pub fn address_map_section(&self) -> &[u8] {
        let offset = self.inner.as_ptr() as usize;
        &self.as_slice()[self.address_map.start - offset..self.address_map.end - offset]
    }
