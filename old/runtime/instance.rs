...
use crate::runtime::builtins::VMBuiltinFunctionsArray;
...
impl Instance {
    pub(crate) unsafe fn new_unchecked(
        ...
        Ok(Self {
            ...
            tables,
            memories,
            dropped_elems: EntitySet::new(),
            dropped_data: EntitySet::new(), // TODO
            // dropped_elems: module.compiled().module.active_table_initializers.clone(),
            // dropped_data: module.compiled().module.active_memory_initializers.clone(),
            exports,
    ...
    fn get_func_ref(&mut self, index: FuncIndex) -> Option<*mut VMFuncRef> {
        ...
        let func = &self.module().compiled().module.functions[index];
        let sig = func.signature;

        unsafe {
            let func_ref: *mut VMFuncRef = self.vmctx.plus_offset_mut::<VMFuncRef>(
                self.module().vmoffsets().vmctx_func_ref(func.func_ref),
            );
    ...
    unsafe fn construct_func_ref(
        ...
        let func_ref = if let Some(def_index) = self.module().compiled().module.defined_func_index(index) {
    ...
    pub fn imported_function(&self, index: FuncIndex) -> &VMFunctionImport {
        unsafe {
            &*self
                .vmctx
                .plus_offset(self.module().vmoffsets().vmctx_function_import(index))
        }
    }

    pub fn get_exported_table(&mut self, index: TableIndex) -> ExportedTable {
        let (definition, vmctx) =
            if let Some(def_index) = self.module().compiled().module.defined_table_index(index) {
        ...
        ExportedTable {
            definition,
            vmctx,
            table: self.module().compiled().module.table_plans[index].clone(),
        }
    }
    pub fn table_ptr(&mut self, index: DefinedTableIndex) -> *mut VMTableDefinition {
        unsafe {
            self.vmctx
                .plus_offset_mut(self.module().vmoffsets().vmctx_table_definition(index))
        }
    }
    pub fn imported_table(&self, index: TableIndex) -> &VMTableImport {
        unsafe {
            &*self
                .vmctx
                .plus_offset(self.module().vmoffsets().vmctx_table_import(index))
        }
    }

    pub fn get_exported_memory(&mut self, index: MemoryIndex) -> ExportedMemory {
        let (definition, vmctx) =
            if let Some(def_index) = self.module().compiled().module.defined_memory_index(index) {
        ...
        ExportedMemory {
            definition,
            vmctx,
            memory: self.module().compiled().module.memory_plans[index].clone(),
        }
    }
    pub fn memory_ptr(&mut self, index: DefinedMemoryIndex) -> *mut VMMemoryDefinition {
        unsafe {
            self.vmctx
                .plus_offset_mut(self.module().vmoffsets().vmctx_memory_definition(index))
        }
    }
    pub fn imported_memory(&self, index: MemoryIndex) -> &VMMemoryImport {
        unsafe {
            &*self
                .vmctx
                .plus_offset(self.module().vmoffsets().vmctx_memory_import(index))
        }
    }

    pub fn get_exported_global(&mut self, index: GlobalIndex) -> ExportedGlobal {
        let (definition, vmctx) =
            if let Some(def_index) = self.module().compiled().module.defined_global_index(index) {
        ...
        ExportedGlobal {
            definition,
            vmctx,
            ty: self.module().compiled().module.globals[index].clone(),
        }
    }
    pub fn global_ptr(&mut self, index: DefinedGlobalIndex) -> *mut VMGlobalDefinition {
        unsafe {
            self.vmctx
                .plus_offset_mut(self.module().vmoffsets().vmctx_global_definition(index))
        }
    }
    pub fn imported_global(&self, index: GlobalIndex) -> &VMGlobalImport {
        unsafe {
            &*self
                .vmctx
                .plus_offset(self.module().vmoffsets().vmctx_global_import(index))
        }
    }
    ...
    pub(crate) unsafe fn vmctx_magic(&self) -> u32 {
        *self
            .vmctx
            .plus_offset::<u32>(self.module.vmoffsets().static_.vmctx_magic())
    }
    pub(crate) unsafe fn vmctx_stack_limit(&self) -> usize {
        *self
            .vmctx
            .plus_offset::<usize>(self.module.vmoffsets().static_.vmctx_stack_limit())
    }
    pub(crate) unsafe fn vmctx_builtin_functions(&self) -> *const VMBuiltinFunctionsArray {
        self.vmctx.plus_offset::<VMBuiltinFunctionsArray>(
            self.module.vmoffsets().static_.vmctx_builtin_functions(),
        )
    }
    pub(crate) unsafe fn vmctx_last_wasm_exit_fp(&self) -> usize {
        *self
            .vmctx
            .plus_offset::<usize>(self.module.vmoffsets().static_.vmctx_last_wasm_exit_fp())
    }
    pub(crate) unsafe fn vmctx_last_wasm_exit_pc(&self) -> usize {
        *self
            .vmctx
            .plus_offset::<usize>(self.module.vmoffsets().static_.vmctx_last_wasm_exit_pc())
    }
    pub(crate) unsafe fn vmctx_last_wasm_entry_fp(&self) -> usize {
        *self
            .vmctx
            .plus_offset::<usize>(self.module.vmoffsets().static_.vmctx_last_wasm_entry_fp())
    }
    pub(crate) unsafe fn vmctx_table_definitions(&self) -> &[VMTableDefinition] {
        slice::from_raw_parts(
            self.vmctx.plus_offset::<VMTableDefinition>(
                self.module.vmoffsets().vmctx_table_definitions_start(),
            ),
            self.module.vmoffsets().num_defined_tables() as usize,
        )
    }
    pub(crate) unsafe fn vmctx_memory_definitions(&self) -> &[VMMemoryDefinition] {
        slice::from_raw_parts(
            self.vmctx.plus_offset::<VMMemoryDefinition>(
                self.module.vmoffsets().vmctx_memory_definitions_start(),
            ),
            self.module.vmoffsets().num_defined_memories() as usize,
        )
    }
    pub(crate) unsafe fn vmctx_global_definitions(&self) -> &[VMGlobalDefinition] {
        slice::from_raw_parts(
            self.vmctx.plus_offset::<VMGlobalDefinition>(
                self.module.vmoffsets().vmctx_global_definitions_start(),
            ),
            self.module.vmoffsets().num_defined_globals() as usize,
        )
    }
    pub(crate) unsafe fn vmctx_func_refs(&self) -> &[VMFuncRef] {
        slice::from_raw_parts(
            self.vmctx
                .plus_offset::<VMFuncRef>(self.module.vmoffsets().vmctx_func_refs_start()),
            self.module.vmoffsets().num_escaped_funcs() as usize,
        )
    }
    pub(crate) unsafe fn vmctx_function_imports(&self) -> &[VMFunctionImport] {
        slice::from_raw_parts(
            self.vmctx.plus_offset::<VMFunctionImport>(
                self.module.vmoffsets().vmctx_function_imports_start(),
            ),
            self.module.vmoffsets().num_imported_funcs() as usize,
        )
    }
    pub(crate) unsafe fn vmctx_table_imports(&self) -> &[VMTableImport] {
        slice::from_raw_parts(
            self.vmctx
                .plus_offset::<VMTableImport>(self.module.vmoffsets().vmctx_table_imports_start()),
            self.module.vmoffsets().num_imported_tables() as usize,
        )
    }
    pub(crate) unsafe fn vmctx_memory_imports(&self) -> &[VMMemoryImport] {
        slice::from_raw_parts(
            self.vmctx.plus_offset::<VMMemoryImport>(
                self.module.vmoffsets().vmctx_memory_imports_start(),
            ),
            self.module.vmoffsets().num_imported_memories() as usize,
        )
    }
    pub(crate) unsafe fn vmctx_global_imports(&self) -> &[VMGlobalImport] {
        slice::from_raw_parts(
            self.vmctx.plus_offset::<VMGlobalImport>(
                self.module.vmoffsets().vmctx_global_imports_start(),
            ),
            self.module.vmoffsets().num_imported_globals() as usize,
        )
    }
}
...
unsafe fn initialize_vmctx(
    ...
    let vmctx_plan = module.vmoffsets();
    *vmctx.plus_offset_mut(vmctx_plan.static_.vmctx_magic()) = VMCONTEXT_MAGIC;

    // Initialize the built-in functions
    *vmctx.plus_offset_mut(vmctx_plan.static_.vmctx_builtin_functions()) =
    ...
    for def_index in module
        ...
        .table_plans
        .keys()
        .filter_map(|index| module.compiled().module.defined_table_index(index))
    {
        let ptr = vmctx
    ...
    for (def_index, plan) in module
        ...
        .memory_plans
        .iter()
        .filter_map(|(index, plan)| Some((module.compiled().module.defined_memory_index(index)?, plan)))
    {
...
unsafe fn initialize_vmctx_globals(
    ...
    for (def_index, init_expr) in module.compiled().module.global_initializers.iter() {
        let val = const_eval.eval(init_expr)?;
        let ptr = vmctx.plus_offset_mut::<VMGlobalDefinition>(
            module.vmoffsets().vmctx_global_definition(def_index),
        );
...
unsafe fn initialize_tables(
    ...
    for (def_index, init) in module.compiled().module.table_initializers.initial_values.iter() {
        let val = match init {
    ...
    // run active elements
    for segment in module.compiled().module.table_initializers.segments.iter() {
        ...
        if let Some(def_index) = module.compiled().module.defined_table_index(segment.table_index) {
...
unsafe fn initialize_memories(
    ...
    for init in module.compiled().module.memory_initializers.iter() {
        let offset = const_eval.eval(&init.offset)?;
        let offset = usize::try_from(offset.get_u64()).unwrap();

        if let Some(def_index) = module.compiled().module.defined_memory_index(init.memory_index) {
