...
impl<T> MmapVec<T> {
    ...
    pub fn new_zeroed(len: usize) -> crate::Result<Self> {
        Ok(Self {
            mmap: Mmap::new(len)?,
            len,
            _m: PhantomData,
    ...
    pub fn with_reserved(capacity: usize) -> crate::Result<Self> {
        Ok(Self {
    ...
    pub(crate) fn try_extend_with(&mut self, count: usize, elem: T) -> crate::Result<()>
    where
        T: Clone,
    {
        let mut tx = self.guard();
