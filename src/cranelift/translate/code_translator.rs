...
use crate::cranelift::translate::state::{ControlStackFrame, ElseData, FuncTranslationState};
use crate::cranelift::translate::utils::{
...
pub fn translate_operator(
    ...
    match op {
        ...
        Operator::CallIndirect {
            ...
            let call = env.translate_call_indirect(
                builder,
        ...
        Operator::GlobalGet { global_index } => {
            ...
            let val = match *state.get_global(builder.func, global_index, env)? {
                ...
                Global::Custom => env.translate_custom_global_get(builder, global_index)?,
            };
            state.push1(val);
        }
        Operator::GlobalSet { global_index } => {
            ...
            match *state.get_global(builder.func, global_index, env)? {
                Global::Const(_) => panic!("global #{global_index:?} is a constant"),
                Global::Memory { gv, offset, ty } => {
                    let addr = builder.ins().global_value(env.pointer_type(), gv);
                    let mut flags = MemFlags::trusted();
                    // Put globals in the "table" abstract heap category as well.
        ...
        Operator::MemoryGrow { mem } => {
            ...
            // env.before_memory_grow(builder, delta, heap_index)?;
            state.push1(env.translate_memory_grow(builder.cursor(), heap_index, delta)?)
        }
        Operator::MemorySize { mem } => {
            let heap_index = MemoryIndex::from_u32(*mem);
            state.push1(env.translate_memory_size(builder.cursor(), heap_index)?);
        }
        ...
        Operator::MemoryFill { mem } => {
            ...
            let val = state.pop1();
            let dest = state.pop1();
            env.translate_memory_fill(builder.cursor(), heap_index, dest, val, len)?;
        }
        ...
        Operator::MemoryAtomicWait32 { memarg } | Operator::MemoryAtomicWait64 { memarg } => {
            ...
            let effective_addr = if memarg.offset == 0 {
                ...
                // TODO let index_type = environ.heaps()[heap].index_type;
                let index_type = I32;
        ...
        Operator::V128Load8x8S { memarg } => {
            ...
            let index = state.pop1();
            let heap = state.get_heap(builder.func, MemoryIndex::from_u32(memarg.memory), env)?;
            let (flags, _, base) = unwrap_or_return_unreachable_state!(
                state,
                heap.prepare_addr(builder, index, 8, memarg, env)?
            );
        ...
        Operator::CallRef { type_index } => {
            ...
            let call = env.translate_call_ref(builder, sigref, callee, state.peekn(num_args))?;
