...
use crate::vm::{self, Imports};
use crate::{Extern, Store};

#[derive(Debug, Clone, Copy)]
pub struct Instance(pub(crate) Stored<vm::Instance>);
...
impl Instance {
    pub(crate) unsafe fn new_unchecked(
        ...
        let instance = vm::Instance::new_unchecked(alloc, const_eval, module, imports)?;
    ...
    pub fn get_export(&self, store: &mut Store, name: &str) -> Option<Extern> {
        let (export_name_index, _, index) = self.module(store).parsed().exports.get_full(name)?;
