    ...
    match op {
        ...
        Operator::GlobalGet { global_index } => {
            ...
            let val = match state.get_global(builder.func, global_index, env)? {
                IRGlobal::Const(val) => val,
                IRGlobal::Memory { gv, offset, ty } => {
                ...
                IRGlobal::Custom => env.translate_custom_global_get(builder, global_index)?,
            };
            state.push1(val);
        }
        Operator::GlobalSet { global_index } => {
            ...
            match state.get_global(builder.func, global_index, env)? {
                IRGlobal::Const(_) => panic!("global #{global_index:?} is a constant"),
                IRGlobal::Memory { gv, offset, ty } => {
                    let addr = builder.ins().global_value(env.pointer_type(), gv);
                    let mut flags = MemFlags::trusted();
                    // Put globals in the "table" abstract heap category as well.
                    ...
                    // env.before_update_global(builder, global_index, val)?;
                }
                IRGlobal::Custom => {
                    let val = state.pop1();
