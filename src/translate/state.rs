...
pub struct FuncTranslationState {
    ...
    // /// Global variables that have been created by `FuncEnvironment::make_global`.
    // tables: HashMap<TableIndex, TranslatedTable>,
    globals: HashMap<GlobalIndex, IRGlobal>,
    memories: HashMap<MemoryIndex, IRHeap>,
    ...
    /// through `TranslationEnvironment::make_direct_func`.
...
impl FuncTranslationState {
    ...
    pub(crate) fn get_direct_func(
        ...
        match self.functions.entry(func_index) {
            ...
            Entry::Vacant(entry) => {
                let fref = env.make_direct_func(func, func_index)?;
                let sig = func.dfg.ext_funcs[fref].signature;
                Ok(*entry.insert((fref, num_wasm_parameters(&func.dfg.signatures[sig], env))))
            }
        }
    }
    ...
    pub(crate) fn get_indirect_sig(
        ...
        match self.signatures.entry(sig_index) {
            Entry::Occupied(entry) => Ok(*entry.get()),
            Entry::Vacant(entry) => {
                let sig = env.make_indirect_sig(func, sig_index)?;
                Ok(*entry.insert((sig, num_wasm_parameters(&func.dfg.signatures[sig], env))))
            }
        }
    }
...
fn num_wasm_parameters(signature: &ir::Signature, environ: &TranslationEnvironment) -> usize {
    (0..signature.params.len())
        .filter(|index| environ.is_wasm_parameter(*index))
        .count()
}
