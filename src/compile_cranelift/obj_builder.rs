...
use crate::compile_cranelift::{
    CompileJobs, CompileOutput, CompiledFunction, CompiledModule, FunctionName, RelocationTarget,
    TrapInfo,
};
use crate::parse::{DebugInfo, ParsedModule, ParsingResult};
use crate::Compiler;
use alloc::boxed::Box;
use alloc::vec::Vec;
use core::ops::Range;
...
impl<'obj> ObjectBuilder<'obj> {
    ...
    pub(crate) fn append(
        ...
        let compile_jobs = CompileJobs::from_module(&module, function_bodies);

        tracing::trace!("Compiling functions to machine code...");
        let unlinked_outputs = compiler.compile_inputs(compile_jobs)?;
        tracing::debug!("{unlinked_outputs:?}");

        tracing::trace!("Appending compiled functions to intermediate code object...");
        let funcs = unlinked_outputs.link_and_append(self, compiler.target_isa(), &module)?;
