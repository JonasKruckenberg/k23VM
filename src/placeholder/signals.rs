...
unsafe extern "C" fn trap_handler(
    ...
    let handled = crate::vm::trap_handling::TLS.with(|p| unsafe {
        ...
        if cfg!(target_os = "macos") {
            unsafe extern "C" fn wasmtime_longjmp_shim(jmp_buf: *const u8) {
                crate::placeholder::longjmp(jmp_buf.cast_mut().cast(), 1)
            }
            set_pc(context, wasmtime_longjmp_shim as usize, info.jmp_buf.as_ptr() as usize);
            return true;
        }
        crate::placeholder::longjmp(info.jmp_buf.as_ptr().cast(), 1)
    });
